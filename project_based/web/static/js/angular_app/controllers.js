...
function get_project_items($scope, $http, parameter, param, from) {
    ...
    if (from == 'dn') {
        ...
        var url = '/project/project_items/?project_id='+$scope.purchase.project_id+'&'+parameter+'='+param
    } else {
...
function validate_add_supplier($scope) {
    ...
    if($scope.name == '' || $scope.name == undefined) {
        ...
        return false;
    } else if (($scope.email != undefined && $scope.email.length>0) && (!(validateEmail($scope.email)))) {
...
function add_new_supplier($scope, $http) {
    if(validate_add_supplier($scope)) {
        ...
        $http({
            ...
            if (data.result == 'error'){
                $scope.error_flag=true;
                $scope.validation_error = data.error_message;
            } else {
    ...
    $http.get(url).success(function(data)
    {
        if (from == 'print') {
            if (data.whole_delivery_notes.length == 0) {
                ...
                if($scope.sales)
                    $scope.sales.sales_items = $scope.delivery_note.sales_items;
            }
        } else {
            $scope.sales = {
                'sales_items': [],
            ...
            if (data.delivery_notes.length == 0) {
                ...
                if (from == 'dn_sales') {
                    ...
                    $scope.sales.dn_id = $scope.delivery_note.id;   
                    $scope.sales.net_total = $scope.delivery_note.net_total; 
                    $scope.calculate_grant_total_sale(); 
                } else {
                    for (var i=0; i<$scope.delivery_note.sales_items.length; i++){
                        $scope.delivery_note.sales_items[i].sold_qty = $scope.delivery_note.sales_items[i].qty_sold;
                    }
                }
                console.log($scope.delivery_note.sales_items);     
            }
        }
...
function PurchaseController($scope, $element, $http, $timeout, share, $location) {
    ...
    $scope.init = function(csrf_token, invoice_number, cash_in_hand)
    {
        $scope.csrf_token = csrf_token;
        ...
        $scope.purchase.cash_in_hand = cash_in_hand;
    }
    ...
    $scope.get_items = function(parameter) {
        if ($scope.purchase.project_id == 'select' || $scope.purchase.project_id == '' ||  $scope.purchase.project_id == undefined) {
            ...
            get_inventory_items($scope, $http, parameter, param, 'purchase');
        }
    }
    ...
    $scope.validate_purchase = function() {
        ...
        if($scope.purchase.supplier_invoice_number == '') {
            ...
            return false;
        } else if(Number($scope.purchase.supplier_amount) > Number(cash_in_hand)) {
            
            $scope.validation_error = "Purchase Amount exceeds Cash In Hand ";
            return false;
...
function ExpenseController($scope, $element, $http, $timeout, $location) {
    ...
    $scope.init = function(csrf_token, cash_in_hand, expense_id)
    {
        $scope.csrf_token = csrf_token;
        get_expense_head_list($scope, $http);
        get_projects($scope, $http);
        $scope.expense.cash_in_hand = cash_in_hand;
...
function ProjectDeliveryNoteController($scope, $element, $http, $timeout, share, $location) {
    ...
    $scope.validate_project_dn = function() {
        ...
        if($scope.delivery_note.delivery_note_number =='' || $scope.delivery_note.delivery_note_number == undefined ){
            ...
            return false;
        } else if($scope.delivery_note.customer =='' || $scope.delivery_note.customer == undefined || $scope.delivery_note.customer =='select' || $scope.delivery_note.customer =='other'){
            $scope.validation_error = "choose customer";
            return false;
            ...
            for (var i=0; i < $scope.delivery_note.sales_items.length; i++){
                if ($scope.delivery_note.sales_items[i].qty_sold <= 0){
                    $scope.validation_error = "Please enter quantity (greater than zero) for all items";
                    return false;
                }
            }
        } 
...
function SalesDNController($scope, $http, $element, $location) {
    ...
    $scope.calculate_net_amount_sale = function(item) {
        ...
        var remaining_qty = parseInt(item.qty_sold) - parseInt(item.sold_qty);
        
        if(item.type == 'item' && parseInt(remaining_qty) > parseInt(item.current_stock)) {
            $scope.validation_error = "Quantity for the item with code "+ item.item_code+ " not in stock";
            return false;
        } else {
            $scope.validation_error = "";
    ...
    $scope.validate_sales = function() {
        ...
        if($scope.sales.dn_id == '' || $scope.sales.dn_id == undefined) {
            ...
            $scope.validation_error ="Please choose payment mode as credit , because you have balance amount.";
            return false;
             // else if ($scope.sales.paid != $scope.sales.grant_total && $scope.sales.payment_mode == 'cash') {
            // $scope.validation_error ="You have choosed cash as payment mode , so please enter the PAID";
            // return false;
        // } else if ((parseFloat($scope.sales.balance) != 0) && ($scope.sales.payment_mode == 'cash' || $scope.sales.payment_mode == 'cheque')) {
            // $scope.validation_error ="Please choose payment mode as credit , because you have balance amount.";
            // return false;
        } else if($scope.sales.sales_items.length > 0){
            for (var i=0; i < $scope.sales.sales_items.length; i++){
                var newly_purchased = parseInt($scope.sales.sales_items[i].qty_sold) - parseInt($scope.sales.sales_items[i].sold_qty);
                if ($scope.sales.sales_items[i].type == 'item' && parseInt($scope.sales.sales_items[i].current_stock) < parseInt(newly_purchased)) {
                    $scope.validation_error = "Quantity not in stock for item with code "+$scope.sales.sales_items[i].item_code;
                    return false;
                }
            }
...
function InventoryPurchaseController($scope, $http, $element, $location) {
    ...
    $scope.validate_purchase = function() {
        ...
        if($scope.purchase.supplier_invoice_number == '') {
            ...
            $scope.validation_error = "Please enter a number as discount";
        } else if(Number($scope.purchase.cash_in_hand) < Number($scope.purchase.supplier_amount)) {
            
            $scope.validation_error = "Please check the Purchase amount with Cash In Hand";
            return false;
        } else if ($scope.purchase.purchase_items.length > 0) {
            for(i=0; i<$scope.purchase.purchase_items.length; i++){
                if ($scope.purchase.purchase_items[i].selling_price == 0 || $scope.purchase.purchase_items[i].selling_price == '') {
                    $scope.validation_error = "Enter selling price for the item with code "+$scope.purchase.purchase_items[i].item_code;
                    return false;
                }else if(parseInt($scope.purchase.purchase_items[i].qty_purchased) == 0 || $scope.purchase.purchase_items[i].qty_purchased == ''){
                    $scope.validation_error = "Please enter quantity ";
                    return false;
                } 
            }
        ...
        if($scope.delivery_note.delivery_note_number =='' || $scope.delivery_note.delivery_note_number == undefined){
            ...
            for (var i=0; i < $scope.delivery_note.sales_items.length; i++){
                if ($scope.delivery_note.sales_items[i].type == 'item' && parseInt($scope.delivery_note.sales_items[i].current_stock) < parseInt($scope.delivery_note.sales_items[i].qty_sold)){
                    $scope.validation_error = "Quantity not in stock for item with code "+$scope.delivery_note.sales_items[i].item_code;
                    return false;
                }else if(parseInt($scope.delivery_note.sales_items[i].qty_sold) == 0){
                    $scope.validation_error = "Please enter quantity ";
                    return false;
                }
            }
...
function EditDeliveryController($scope, $element, $http, $timeout, share, $location) {
    ...
    $scope.calculate_net_amount_sale = function(item) {
        ...
        if(item.type == 'item' && parseInt(newly_purchased) > parseInt(item.current_stock)) {
            $scope.validation_error = "Quantity not in stock";
            return false;
        } else {
            $scope.validation_error = "";
    ...
    $scope.delivery_note_validation = function(){
        if ($scope.delivery_note_no == '' || $scope.delivery_note_no == undefined) {
            ...
            for (var i=0; i < $scope.delivery_note.sales_items.length; i++){
                var newly_purchased = parseInt($scope.delivery_note.sales_items[i].qty_sold) - parseInt($scope.delivery_note.sales_items[i].sold_qty);
                if ($scope.delivery_note.sales_items[i].type == 'item' && parseInt($scope.delivery_note.sales_items[i].current_stock) <  parseInt(newly_purchased)){
                    $scope.validation_error = "Quantity not in stock for item "+$scope.delivery_note.sales_items[i].item_code;
                    return false;
                } 
            }
        } 
...
function EditItemController($scope, $http, $element) {
    $scope.item = {
        'name': '',
        'code': '',
        'type': '',
    ...
    $scope.get_item = function(){
        $http.get('/project/edit_item/'+$scope.item_id+"/").success(function(data){
            $scope.item = data;
        });
    }
    ...
    function add_item($scope, $http, from) {
        ...
        if ($scope.is_valid) {
            ...
            $http({
                method : 'post',
                url : '/project/edit_item/'+$scope.item_id+"/",
                data : $.param(params),
                headers : {
                    'Content-Type' : 'application/x-www-form-urlencoded'
                }
...
function CashInHandEntryController($scope, $element, $http) {
    ...
    $scope.cash_entry_form_validation = function() {
        $scope.cash_entry.date = $$('#date')[0].get('value');
        if ($scope.cash_entry.date == '' || $scope.cash_entry.date == undefined) {
            ...
            return false;
        } else if ($scope.is_other_purpose && ($scope.cash_entry.other_purpose == '' || $scope.cash_entry.other_purpose == undefined)) {
            $scope.validation_error = 'Please enter other purpose';
            return false;
        } else if ($scope.cash_entry.amount == '' || $scope.cash_entry.amount == undefined || $scope.cash_entry.amount != Number($scope.cash_entry.amount)) {
        ...
        if($scope.is_valid) {
            ...
            $http({
                ...
                if (data.result == 'error'){
                    ...
                    document.location.href = '/cash_in_hand_entry/';   
...
function ViewPurchaseController($scope, $http, $element) {
    $scope.init = function(csrf_token) {
        $scope.csrf_token = csrf_token;
    }
    $scope.load_purchase_details = function() {
        $http.get('/purchase/purchase_details/?purchase_invoice_no='+$scope.purchase.purchase_invoice_number).success(function(data){
            if (data.result != 'error') {
                $scope.purchase = data.purchase;
            } else {
                $scope.purchase = {};
                $scope.error_message = 'No purchase with this invoice number';
            }
        });
    }
}

function OutstandingCustomerReportController($scope, $http, $element) {
    $scope.init = function(csrf_token,report_type) {
        $scope.csrf_token = csrf_token;
        get_customers($scope, $http);
        $scope.customer = 'select';
